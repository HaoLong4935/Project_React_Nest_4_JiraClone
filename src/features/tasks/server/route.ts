...
const app = new Hono()
    .get("/",
        ...
        async (c) => {
            ...
            // Populate projects data and members data using Ids
    ...
    .post("/bulk-update",
        ...
        async (c) => {
            ...
            // Security check : Only valid members are allowed
            const workspaceIds = new Set(tasksToUpdate.documents.map(task => task.workspaceId))
            if (workspaceIds.size !== 1) {
                return c.json({ error: "All task must belong to the same workspace Id" })
            }
